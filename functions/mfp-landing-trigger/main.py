...
def extract_file_dt(name):
    ...
    m = re.search(
        r"(?P<year>2[0-9]{3})[-_.]?(?P<month>[0-1]{1}[0-9]{1}|JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[-_.]?(?P<day>[0-3]{1}[0-9]{1})[-_.]?((?P<hour>[0-2]{1}[0-3]{1})[-_.]?(?P<minute>[0-5]{1}[0-9]{1}))?",
        name,
        re.IGNORECASE,
    )
    ...
    if not m:
        m = re.search(
            r"(?P<day>[0-3]{1}[0-9]{1})[-_.]?(?P<month>[0-1]{1}[0-9]{1}|JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[-_.]?(?P<year>2[0-9]{3})[-_.]?((?P<hour>[0-2]{1}[0-3]{1})[-_.]?(?P<minute>[0-5]{1}[0-9]{1}))?",
            name,
            re.IGNORECASE,
        )
    ...
    if m.group("hour"):
        ...
        file_dt = str(
            datetime(int(m.group("year")), int(m.group("month")), int(m.group("day")))
        )
